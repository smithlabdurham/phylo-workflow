...
rbOff <- lapply(rbTrees, function(trees) {
  nTree <- length(trees)
  trees[(1 + (nTree * 0)):nTree]
})
rbBurntOff<- RootTree(do.call(c, rbOff), outgroup)
...
rbSample <- lapply(rbOff, Sample, bayesSample)
```
...
calcPP <- sum(lengths(c(burntOff, rbOff))) > 0
...
for (i in as.integer(names(cl[order(cl, decreasing = TRUE)]))) {
  ...
  legend("bottomleft",
         ...
         bty = "n")
  if (calcPP) {
    splitFreqs <- SplitFrequency(cons, c(burntOff, rbBurntOff)) / 
      length(c(burntOff, rbBurntOff))
    tooBright <- 26
