...
#| eval = !is.null(rbTrees),
#| message = FALSE,
#| results = "hide"
set.seed(1) # Ensure consistent result from heuristic search
# This will display any rogue taxa found
rogues <- Rogue::QuickRogue(rbTrees)
rogues
```
...
```
(ref:pars-lowK-cap) Strict consensus of results for implied weights parsimony with low concavity values (`r paste(lowK, collapse = ", ")`).  Splits are labelled with their quartet concordance [@Minh2020mbe].

```{r plot-pars-lowK-rogue, echo = FALSE}
#| eval = evalLowK && nrow(rogues) > 1,
#| message = FALSE,
#| fig.height = 6.4,
...
if (length(rogueTaxa) == 1) {
  ...
  PlotTools::SpectrumLegend(
    ...
    cex = 0.4, bty = "n", title = paste("Position of", rogueTaxa, "in")
  )
} else {
...
```{r ew-rogues, echo = FALSE, results = "hide"}
#| eval = evalEW
set.seed(1) # Ensure consistent result from heuristic search
...
calcPP <- sum(lengths(c(burntOff, rbTrees))) > 0
...
for (i in as.integer(names(cl[order(cl, decreasing = TRUE)]))) {
  ...
  legend("bottomleft",
         ...
         bty = "n")
  if (calcPP) {
    splitFreqs <- SplitFrequency(cons, c(burntOff, rbTrees)) / 
      length(c(burntOff, rbTrees))
    tooBright <- 26
